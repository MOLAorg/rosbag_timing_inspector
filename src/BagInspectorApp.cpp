...
void BagInspectorApp::render_timeline_tab()
{
    ...
    ImGui::BeginChild("topic_list", ImVec2(220, 0), true);
    ...
    for (size_t i = 0; i < n_topics; i++)
    {
        ...
        // Show msg count and mean frequency as a tooltip
    ...
    if (m_measure_active)
    {
        ...
        if (m_measure_has_second)
        {
            ...
            ImGui::SameLine();
            ImGui::TextColored(
                ImVec4(1, 1, 0, 1), "  DeltaT = %.6f s  (%.3f ms)",
                dt, dt * 1e3);
        }
        else if (m_measure_active)
        {
            ImGui::SameLine();
...
void BagInspectorApp::render_histograms_tab()
{
    ...
    // Two columns: histograms on left, stats table on right
    ...
    for (size_t i = 0; i < n_topics; i++)
    {
        ...
        if (ImPlot::BeginPlot(label, ImVec2(-1, plot_h)))
        {
            ...
            ImPlot::PlotHistogram(
                ttd.topic_name.c_str(),
                ttd.intervals_s.data(),
                static_cast<int>(ttd.intervals_s.size()),
                ImPlotBin_Sqrt,  // auto bin count
                1.0,             // bar scale
...
void BagInspectorApp::render()
{
    ...
    if (m_data.empty())
    {
        ImGui::TextDisabled("No bag loaded. Use File > Open bag... or pass a path on the command line.");
    }
    else
    {
        ImGui::Text(
            "Bag: %s   |   Duration: %.3f s   |   Topics: %zu",
            m_data.bag_path.c_str(),
            m_data.duration_s(),
            m_data.topics.size());
        ImGui::Separator();
    ...
    if (ImGui::BeginPopupModal(
            "Error##modal", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
    {
